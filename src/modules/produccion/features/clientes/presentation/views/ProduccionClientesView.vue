...
import {
  useProduccionClientsList,
  useProduccionClientStats,
  useDeleteProduccionClient,
} from '../../application/useClients'
import type { ClientListItem, ListClientsParams } from '../../domain/client.types'
import { produccionClientsRepository } from '@modules/produccion/features/clientes'
...
const {
  ...
  refetch: refetchList,
} = useProduccionClientsList(listParams)
const {
  data: stats,
...
} = useProduccionClientStats(PRODUCCION_APP_SLUG)
...
const goToNew = () => router.push(`${PRODUCCION_BASE_PATH}/clientes/nuevo`)
const goToDetail = (client: ClientListItem) =>
  router.push(`${PRODUCCION_BASE_PATH}/clientes/${client.id}`)
const goToEdit = (client: ClientListItem) =>
  router.push(`${PRODUCCION_BASE_PATH}/clientes/${client.id}/editar`)

const showConfirmModal = ref(false)
const confirmClient = ref<ClientListItem | null>(null)
const { mutate: deleteClient, isPending: isDeletingClient } = useDeleteProduccionClient()
...
async function handleExport() {
  const result = await produccionClientsRepository.getList({
    applicationSlug: PRODUCCION_APP_SLUG,
    page: 1,
    limit: 10000,
    search: searchInput.value.trim() || undefined,
  ...
  await exportToExcel({
    fileName: `clientes_produccion_${now}`,
    sheetName: 'Clientes',
    columns: [
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        ...
        <p class="text-sm mt-1" :style="{ color: 'var(--color-text-secondary)' }">
          Clientes residenciales y corporativos — cotizaciones, pedidos y entregas
        </p>
      </div>
        ...
        <template v-else>
          ...
          >
            ...
            <template #row="{ row }">
              ...
              <td class="py-3 px-4 text-sm" :style="{ color: 'var(--color-text-secondary)' }">
                <span class="inline-flex items-center gap-1">
                  <AppIcon icon="lucide:package" :size="16" />
                  —
                </span>
              </td>
              <td class="py-3 px-4">
