...
import {
  ...
  useProvinces,
  useDistricts,
  useUpdateProduccionClient,
} from '../../application/useClients'
import type { DocumentType, UpdateProduccionClientPayload } from '../../domain/client.types'
...
const { data: client, isLoading: loadingClient, isError: clientError } = useProduccionClient(id)
...
const updateMutation = useUpdateProduccionClient()
...
watch(
  () => clientError.value,
  (isErr) => {
    if (isErr) void router.replace(`${PRODUCCION_BASE_PATH}/clientes`)
  },
)
...
const onSubmit = handleSubmit(async (formValues) => {
  try {
    ...
    const data: UpdateProduccionClientPayload = {
    ...
    router.push(`${PRODUCCION_BASE_PATH}/clientes/${id.value}`)
  } catch {
    void 0
  }
})
