...
import {
  ...
  useProvinces,
  useDistricts,
  useUpdateInteriorClient,
} from '../../application/useClients'
import type { DocumentType, UpdateInteriorClientPayload } from '../../domain/client.types'
...
const { data: client, isLoading: loadingClient, isError: clientError } = useInteriorClient(id)
...
const updateMutation = useUpdateInteriorClient()
...
watch(
  ...
  (c) => {
    ...
    if (c.clientType !== 'RESIDENTIAL' && c.clientType !== 'CORPORATE') {
      void router.replace(`${INTERIORISMO_BASE_PATH}/clientes`)
      return
    }
...
watch(
  () => clientError.value,
  (isErr) => {
    if (isErr) void router.replace(`${INTERIORISMO_BASE_PATH}/clientes`)
  },
)
...
const onSubmit = handleSubmit(async (formValues) => {
  try {
    ...
    const data: UpdateInteriorClientPayload = {
    ...
    router.push(`${INTERIORISMO_BASE_PATH}/clientes/${id.value}`)
  } catch {
    void 0
  }
})
