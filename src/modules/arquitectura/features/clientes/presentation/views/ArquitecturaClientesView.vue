...
import {
  useArquitecturaClientsList,
  useArquitecturaClientStats,
  useDeleteArquitecturaClient,
} from '../../application/useClients'
import type { ClientListItem, ListClientsParams } from '../../domain/client.types'
import { arquitecturaClientsRepository } from '@modules/arquitectura/features/clientes'
...
const {
  ...
  refetch: refetchList,
} = useArquitecturaClientsList(listParams)
const {
  data: stats,
...
} = useArquitecturaClientStats(ARQUITECTURA_APP_SLUG)
...
const goToNew = () => router.push(`${ARQUITECTURA_BASE_PATH}/clientes/nuevo`)
const goToDetail = (client: ClientListItem) =>
  router.push(`${ARQUITECTURA_BASE_PATH}/clientes/${client.id}`)
const goToEdit = (client: ClientListItem) =>
  router.push(`${ARQUITECTURA_BASE_PATH}/clientes/${client.id}/editar`)

const showConfirmModal = ref(false)
const confirmClient = ref<ClientListItem | null>(null)
const { mutate: deleteClient, isPending: isDeletingClient } = useDeleteArquitecturaClient()
...
async function handleExport() {
  const result = await arquitecturaClientsRepository.getList({
    applicationSlug: ARQUITECTURA_APP_SLUG,
    page: 1,
    limit: 10000,
    search: searchInput.value.trim() || undefined,
