...
import type { ArquitecturaProjectPaymentDto } from '@modules/arquitectura/features/proyectos/domain/project.types'
import {
...
const paySchema = yup.object({
  ...
  amount: yup.number().typeError('Monto invÃ¡lido').min(0.01, 'El monto debe ser mayor a cero'),
...
function openEditPayment(row: ArquitecturaProjectPaymentDto) {
...
async function removePayment(id: string) {
  const ok = await markapAlert.confirmDanger({ title: 'Â¿Eliminar pago?', confirmText: 'Eliminar' })
  if (!ok) return
...
<template>
  <div class="space-y-6">
    ...
    <div v-else-if="isError || !settlement" class="text-center py-12 text-sm" :style="{ color: 'var(--color-text-muted)' }">
      No se pudo cargar la liquidaciÃ³n.
    </div>
    <template v-else>
      ...
      <FormSectionCard
        ...
        subtitle="Equivalente al bloque de liquidaciÃ³n del Excel"
      ...
      <FormSectionCard
        ...
        subtitle="Abonos a cuenta, saldo y pagos finales â€” alimentan la liquidaciÃ³n"
      ...
      >
        ...
        >
          <template #row="{ row }">
            <td class="py-2 px-3 text-sm">{{ (row as ArquitecturaProjectPaymentDto & { paidAt: string }).paidAt }}</td>
            <td class="py-2 px-3 text-sm max-w-[240px] truncate">{{ (row as ArquitecturaProjectPaymentDto).concept }}</td>
            <td class="py-2 px-3">
              <Badge variant="neutral">
                {{ PAYMENT_TYPE_LABELS[(row as ArquitecturaProjectPaymentDto).paymentType ?? 'OTHER'] ?? 'Otro' }}
              </Badge>
            </td>
            <td class="py-2 px-3 text-sm font-medium">{{ (row as { amount: string }).amount }}</td>
            <td class="py-2 px-3">
              <Badge :variant="(row as ArquitecturaProjectPaymentDto).status === 'PAID' ? 'success' : 'warning'">
                {{ (row as ArquitecturaProjectPaymentDto).status }}
              </Badge>
            </td>
                ...
                :items="[
                  {
                    label: 'Editar',
                    icon: 'lucide:pencil',
                    onClick: () => openEditPayment(row as ArquitecturaProjectPaymentDto),
                  },
                  {
                    label: 'Eliminar',
                    icon: 'lucide:trash-2',
                    danger: true,
                    onClick: () => removePayment((row as ArquitecturaProjectPaymentDto).id),
                  },
                ]"
              />
            </td>
    ...
    <BaseModal v-model="payModal" :title="payEditId ? 'Editar pago' : 'Registrar pago'" size="lg">
      <form :id="PAYMENT_FORM_ID" class="space-y-3" @submit.prevent="onSubmitPayment">
        ...
        <FormInput v-bind="payBinds.concept" label="Concepto" placeholder="Ej. Transferencia â€” abono a cuenta" />
