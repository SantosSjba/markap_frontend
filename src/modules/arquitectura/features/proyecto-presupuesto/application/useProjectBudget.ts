...
export const projectBudgetKeys = {
  all: ['arquitectura-project-budget', ARQUITECTURA_APP_SLUG] as const,
...
export function useCreateBudgetSection(projectId: Ref<string> | string) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      void markapAlert.toast.success('SecciÃ³n creada')
    },
    onError: (err) => void markapAlert.toast.error('No se pudo crear la secciÃ³n', getApiErrorMessage(err)),
  })
}
...
export function useUpdateBudgetSection(projectId: Ref<string> | string) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      invalidateBudget(qc, unref(projectId))
      void markapAlert.toast.success('SecciÃ³n actualizada')
    },
...
export function useDeleteBudgetSection(projectId: Ref<string> | string) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      invalidateBudget(qc, unref(projectId))
      void markapAlert.toast.success('SecciÃ³n eliminada')
    },
...
export async function openProjectBudgetPdf(projectId: string): Promise<void> {
  try {
    ...
    void markapAlert.toast.info('Use Imprimir â†’ Guardar como PDF en el navegador.')
  } catch (err) {
    ...
    onSuccess: (result) => {
      ...
      const extra =
        result.unmatchedConcepts.length > 0
          ? ` Â· ${result.unmatchedConcepts.length} concepto(s) sin partida coincidente`
          : ''
      void markapAlert.toast.success(`${result.updatedLineItems} partida(s) actualizada(s)${extra}`)
    },
    onError: (err) =>
      void markapAlert.toast.error('No se pudo sincronizar desde ejecuciÃ³n', getApiErrorMessage(err)),
  })
}
