...
import { useArquitecturaProjectDetail, useUpdateArquitecturaProject } from '../../application/useArquitecturaProjects'
...
const { data: project, isLoading: loadingProject, isError } = useArquitecturaProjectDetail(projectId)
...
const { filterOptions: statusOptions } = useArquitecturaProjectStageOptions({ includeCancelled: true })
  ...
  (p) => {
    ...
    resetForm({
      values: {
        ...
        city: p.city ?? '',
        interventionLevel: p.interventionLevel ?? '',
        executionTimeNote: p.executionTimeNote ?? '',
        currency: p.currency ?? 'PEN',
        defaultUtilityPct: p.defaultUtilityPct ?? 20,
        defaultIgvPct: p.defaultIgvPct ?? 18,
        ...
        projectedCost: p.projectedCost ?? undefined,
        expectedMargin: p.expectedMargin ?? undefined,
      },
    })
  },
  { immediate: true },
...
const updateMut = useUpdateArquitecturaProject()

const loading = computed(() => loadingClients.value || loadingAgents.value || loadingProject.value)

const goBack = () => router.push(`${ARQUITECTURA_BASE_PATH}/proyectos`)

const emptyToUndef = (s: string | undefined | null) => (s?.trim() ? s.trim() : undefined)
...
const onSubmit = handleSubmit(async (v) => {
  ...
  try {
    ...
    router.push(`${ARQUITECTURA_BASE_PATH}/proyectos/${project.value.id}`)
  } catch {
    void 0
  }
})
...
<template>
  <div class="px-3 sm:px-5 py-6 sm:py-8 max-w-[1100px] mx-auto space-y-8">
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-xl font-bold" :style="{ color: 'var(--color-text-primary)' }">Editar proyecto</h1>
        <p class="text-sm mt-0.5" :style="{ color: 'var(--color-text-secondary)' }">
          {{ project?.code ? `Código: ${project.code}` : 'Cargando…' }}
        </p>
      </div>
    </div>
    ...
    <form v-else class="space-y-8" @submit.prevent="onSubmit">
      ...
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="md:col-span-2">
            <p class="text-xs font-medium mb-1" :style="{ color: 'var(--color-text-muted)' }">Código</p>
            <p class="text-sm font-mono font-semibold" :style="{ color: 'var(--color-text-primary)' }">
              {{ project.code }}
            </p>
            <p class="text-xs mt-1" :style="{ color: 'var(--color-text-muted)' }">
              El código no se puede cambiar tras crear el proyecto.
            </p>
          </div>
      ...
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <FormInput v-bind="areaSqmB" type="number" label="Área m²" :error="errors.areaSqm" />
          <FormInput v-bind="levelsCountB" type="number" label="Niveles / pisos" :error="errors.levelsCount" />
          <FormTextarea
      ...
      <section
        ...
        :style="{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)' }"
      >
        <h2 class="text-base font-semibold" :style="{ color: 'var(--color-text-primary)' }">Responsables</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormSelect
            v-bind="designerB"
            label="Diseñador"
      ...
      <div class="flex justify-end gap-3">
        ...
        <BaseButton type="submit" variant="primary" :loading="updateMut.isPending.value">
          Guardar cambios
        </BaseButton>
      </div>
    </form>
