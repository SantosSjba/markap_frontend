...
import { useArquitecturaProjectsList, useUpdateArquitecturaProject } from '@modules/arquitectura/features/proyectos/application/useArquitecturaProjects'
import type { ArquitecturaProjectListItem, ListArquitecturaProjectsParams } from '@modules/arquitectura/features/proyectos/domain/project.types'
...
const listParams = ref<ListArquitecturaProjectsParams>({
  page: 1,
  limit: ITEMS,
  inProgressOnly: true,
})
...
const columns = [
  { key: 'code', label: 'CÃ³digo', align: 'left' as const, sortable: true },
...
const openBoard = (r: ArquitecturaProjectListItem) => {
  router.push(`${ARQUITECTURA_BASE_PATH}/ejecucion/${r.id}`)
}

const goDetail = (r: ArquitecturaProjectListItem) =>
  router.push(`${ARQUITECTURA_BASE_PATH}/proyectos/${r.id}`)
const goEdit = (r: ArquitecturaProjectListItem) =>
  router.push(`${ARQUITECTURA_BASE_PATH}/proyectos/${r.id}/editar`)

const showCancelModal = ref(false)
const cancelTarget = ref<ArquitecturaProjectListItem | null>(null)
const { mutateAsync: updateProject, isPending: isCancelPending } = useUpdateArquitecturaProject()

const openCancelConfirm = (r: ArquitecturaProjectListItem) => {
...
const getActions = (r: ArquitecturaProjectListItem): { label: string; icon: string; onClick: () => void }[] => {
...
const goProjects = () => router.push(`${ARQUITECTURA_BASE_PATH}/proyectos`)
</script>
...
<template>
  <div class="px-3 sm:px-5 py-6 sm:py-8 max-w-[1400px] mx-auto space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        <h1 class="text-xl font-bold" :style="{ color: 'var(--color-text-primary)' }">EjecuciÃ³n</h1>
        ...
        >
          <template #toolbar>
            <div class="flex-1 min-w-0">
              <SearchInput v-model="searchInput" placeholder="Buscar por cÃ³digo o nombreâ€¦" />
            </div>
          </template>
          <template #row="{ row }">
            <td class="py-3 px-4">
              <button
                ...
                @click="goDetail(row as ArquitecturaProjectListItem)"
              >
                {{ (row as ArquitecturaProjectListItem).code }}
              </button>
            </td>
            <td class="py-3 px-4">
              <button type="button" class="text-left font-medium" @click="goDetail(row as ArquitecturaProjectListItem)">
                <span :style="{ color: 'var(--color-text-primary)' }">{{ (row as ArquitecturaProjectListItem).name }}</span>
              </button>
            </td>
            <td class="py-3 px-4 text-sm" :style="{ color: 'var(--color-text-secondary)' }">
              {{ (row as ArquitecturaProjectListItem).client.fullName }}
            </td>
            <td class="py-3 px-4 text-sm">{{ Math.round((row as ArquitecturaProjectListItem).progressPct) }}%</td>
            <td class="py-3 px-4">
              <Badge variant="neutral">{{ projectStatusLabel((row as ArquitecturaProjectListItem).status, stageLabelMap) }}</Badge>
            </td>
            <td class="py-3 px-4 text-right">
              <ActionsDropdown :items="getActions(row as ArquitecturaProjectListItem)" />
            </td>
          </template>
        </DataTable>
    ...
    <BaseModal v-model="showCancelModal" :closable="true" size="sm" @close="closeCancelModal">
      ...
      <div class="p-4 space-y-3">
        <p class="text-sm" style="color: var(--color-text-secondary);">
          Â¿Marcar como cancelado el proyecto
