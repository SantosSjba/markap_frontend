...
export const arquitecturaExecutionKeys = {
  all: ['arquitectura-execution', ARQUITECTURA_APP_SLUG] as const,
  overview: (projectId: string) => [...arquitecturaExecutionKeys.all, 'overview', projectId] as const,
}

export function useArquitecturaExecutionOverview(projectId: Ref<string> | string) {
  const pid = () => unref(projectId)
  return useQuery({
    queryKey: computed(() => arquitecturaExecutionKeys.overview(pid())),
...
export function usePatchExecutionProgress(projectId: Ref<string> | string) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: arquitecturaExecutionKeys.overview(id) })
      invalidateQuerySubtree(qc, arquitecturaProjectKeys.detail(id))
