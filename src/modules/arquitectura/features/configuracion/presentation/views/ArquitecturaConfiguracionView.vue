...
const tabs = [
  { id: 'accesos', label: 'Módulo Arquitectura', icon: 'lucide:layout-dashboard' },
...
const quickLinks = [
  {
    to: `${ARQUITECTURA_BASE_PATH}/clientes`,
    label: 'Clientes',
    icon: 'lucide:users',
    desc: 'Cartera de clientes del módulo',
  },
  {
    to: `${ARQUITECTURA_BASE_PATH}/proyectos`,
    label: 'Proyectos',
    icon: 'lucide:folder-kanban',
    desc: 'Expedientes y obras en curso',
  },
  {
    to: `${ARQUITECTURA_BASE_PATH}/reportes`,
    label: 'Reportes',
...
const { data: boot, isLoading, isError, error, refetch } = useArquitecturaConfigBootstrap()
const configLoadError = computed(() => (isError.value ? getApiErrorMessage(error.value) : ''))
const { mutate: saveStages, isPending: savingStages } = useArquitecturaSaveProjectStages()
const { mutate: saveNumbering, isPending: savingNum } = useArquitecturaSaveArquitecturaProjectNumbering()

const pipelineDraft = ref<ArquitecturaProjectStageDTO[]>([])
...
const {
  ...
  initialValues: { prefix: 'ARQ-PRY', lastNumber: 0 },
})
...
watch(
  () => boot.value?.numbering.arquitecturaProject,
  (n) => {
    if (n) {
...
const flowSteps = [
  { label: 'Cliente', path: `${ARQUITECTURA_BASE_PATH}/clientes`, icon: 'lucide:user' },
  { label: 'Proyecto', path: `${ARQUITECTURA_BASE_PATH}/proyectos`, icon: 'lucide:folder-kanban' },
  { label: 'Presupuesto', path: `${ARQUITECTURA_BASE_PATH}/presupuestos`, icon: 'lucide:file-text' },
  { label: 'Cronograma', path: `${ARQUITECTURA_BASE_PATH}/cronograma`, icon: 'lucide:calendar' },
  { label: 'Reportes', path: `${ARQUITECTURA_BASE_PATH}/reportes`, icon: 'lucide:line-chart' },
]
</script>
  ...
  <div class="space-y-6 max-w-5xl">
    <div>
      ...
      <p class="text-sm mt-1" :style="{ color: 'var(--color-text-secondary)' }">
        Ajustes propios de <strong>Arquitectura</strong> (independiente de Ventas u otros módulos): etiquetas del
    ...
    <div v-if="activeTab === 'accesos'" class="space-y-4">
      <p class="text-sm" :style="{ color: 'var(--color-text-muted)' }">
        Accesos a pantallas frecuentes del módulo (datos filtrados por aplicación arquitectura en backend).
      </p>
    ...
    <div v-else-if="activeTab === 'parametros'" class="space-y-8">
      ...
      <template v-else>
        ...
        <section class="space-y-3">
          ...
          <p class="text-sm" :style="{ color: 'var(--color-text-secondary)' }">
            Referencia para el correlativo de códigos de proyecto (ej. ARQ-PRY-0007). La numeración se usa como guía;
          ...
          <div
            ...
            :style="{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }"
          >
            <p v-if="boot?.numbering.arquitecturaProject" class="text-sm" :style="{ color: 'var(--color-text-muted)' }">
              Próximo código según servidor:
              <strong :style="{ color: 'var(--color-text-primary)' }">{{
                boot.numbering.arquitecturaProject.nextPreview
              }}</strong>
            </p>
        ...
        <section class="space-y-3">
          ...
          <p class="text-sm" :style="{ color: 'var(--color-text-secondary)' }">
            ...
            roles de tu organización; los proyectos Arquitectura solo admiten agentes dados de alta en esta aplicación.
          </p>
        </section>
      </template>
    </div>
    ...
    <div v-else-if="activeTab === 'flujo'" class="space-y-4">
      <p class="text-sm" :style="{ color: 'var(--color-text-secondary)' }">
        Referencia del flujo en Arquitectura (cada paso enlaza al área correspondiente).
      </p>
      <div
