...
import type { ArquitecturaCatalogMaterialListItem, ListArquitecturaCatalogMaterialsParams } from '../../domain/catalog.types'
import {
  useArquitecturaCatalogMaterialsList,
...
const listParams = ref<ListArquitecturaCatalogMaterialsParams>({
  page: 1,
  limit: ITEMS,
})
...
const {
  ...
  refetch: refetchList,
} = useArquitecturaCatalogMaterialsList(listParams)
...
const goDetail = (r: ArquitecturaCatalogMaterialListItem) =>
  router.push(`${ARQUITECTURA_BASE_PATH}/materiales/catalogo/${r.id}`)
const goNew = () => router.push(`${ARQUITECTURA_BASE_PATH}/materiales/catalogo/nuevo`)

const showDeleteModal = ref(false)
const deleteTarget = ref<ArquitecturaCatalogMaterialListItem | null>(null)
const { mutateAsync: deleteMaterial, isPending: isDeletingMaterial } = useDeleteArquitecturaCatalogMaterial()

const openDeleteConfirm = (r: ArquitecturaCatalogMaterialListItem) => {
...
const getActions = (r: ArquitecturaCatalogMaterialListItem): { label: string; icon: string; onClick: () => void }[] => [
          ...
          <template #row="{ row }">
            <td class="py-3 px-4">
              <button
                ...
                @click="goDetail(row as ArquitecturaCatalogMaterialListItem)"
              >
                {{ (row as ArquitecturaCatalogMaterialListItem).code }}
              </button>
            </td>
            <td class="py-3 px-4">
              <button type="button" class="text-left" @click="goDetail(row as ArquitecturaCatalogMaterialListItem)">
                <span class="font-medium" :style="{ color: 'var(--color-text-primary)' }">
                  {{ (row as ArquitecturaCatalogMaterialListItem).name }}
                </span>
              </button>
            </td>
            <td class="py-3 px-4 text-sm" :style="{ color: 'var(--color-text-secondary)' }">
              {{ (row as ArquitecturaCatalogMaterialListItem).category }}
            </td>
            <td class="py-3 px-4 text-sm" :style="{ color: 'var(--color-text-secondary)' }">
              {{ (row as ArquitecturaCatalogMaterialListItem).brand }}
            </td>
            <td class="py-3 px-4 text-sm">{{ (row as ArquitecturaCatalogMaterialListItem).unit }}</td>
            <td class="py-3 px-4 text-sm">{{ formatSol((row as ArquitecturaCatalogMaterialListItem).price) }}</td>
            <td class="py-3 px-4 text-sm">{{ formatQty((row as ArquitecturaCatalogMaterialListItem).stock) }}</td>
            <td class="py-3 px-4 text-sm">{{ (row as ArquitecturaCatalogMaterialListItem).imageCount }}</td>
            <td class="py-3 px-4 text-right">
              <ActionsDropdown :items="getActions(row as ArquitecturaCatalogMaterialListItem)" />
            </td>
          </template>
        </DataTable>
