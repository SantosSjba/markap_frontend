...
import type { ArquitecturaFinanceScheduleDto } from '../../domain/finance.types'
import {
...
} from '../../application/useArquitecturaFinance'
import {
...
const props = withDefaults(
  defineProps<{
    /** Cuando se incrusta en el detalle de proyecto */
    projectId?: string
    embedded?: boolean
  }>(),
  { embedded: false },
...
const projectId = computed(
  () => props.projectId ?? String(route.params.id ?? route.params.projectId ?? ''),
)
...
const { data: ov, isLoading, isError, error: overviewError, refetch: refetchOverview } = useArquitecturaFinanceOverview(projectId)
...
const goHub = () => router.push(`${ARQUITECTURA_BASE_PATH}/proyectos`)
const goProject = () => {
  const id = projectId.value
  if (id) router.push(`${ARQUITECTURA_BASE_PATH}/proyectos/${id}`)
}
...
function openEditSchedule(row: ArquitecturaFinanceScheduleDto) {
...
async function removeSchedule(row: ArquitecturaFinanceScheduleDto) {
...
const waiveTarget = ref<ArquitecturaFinanceScheduleDto | null>(null)
const waiveSubmitting = ref(false)

async function openWaive(row: ArquitecturaFinanceScheduleDto) {
...
<template>
  <div
    :class="
      embedded
        ? 'space-y-6'
        : 'px-3 sm:px-5 py-6 sm:py-8 max-w-[1600px] mx-auto space-y-6'
    "
  >
    <div v-if="!embedded" class="flex flex-wrap items-start gap-3">
      <button
        type="button"
            ...
            >
              <template #row="{ row }">
                ...
                <td class="py-2 px-3 text-right space-x-2 whitespace-nowrap">
                  <button type="button" class="text-xs underline" @click="openEditSchedule(row as ArquitecturaFinanceScheduleDto)">
                    Editar
                  </button>
                  <button
                    v-if="(row as ArquitecturaFinanceScheduleDto).status !== 'WAIVED'"
                    type="button"
                    class="text-xs underline text-amber-700"
                    @click="openWaive(row as ArquitecturaFinanceScheduleDto)"
                  >
                    Condonar
                  </button>
                  <button type="button" class="text-xs text-red-600 underline" @click="removeSchedule(row as ArquitecturaFinanceScheduleDto)">
                    Quitar
                  </button>
                </td>
              </template>
