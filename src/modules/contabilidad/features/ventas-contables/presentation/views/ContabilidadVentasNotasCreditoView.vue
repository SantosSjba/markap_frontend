...
import type { ContabilidadSalesCreditNoteDTO } from '../../domain/sales.types'
...
const { data, isLoading, refetch } = useContabilidadSalesCreditNotes(listParams)
...
const { mutate: createCreditNote, isPending: saving } = useContabilidadCreateSalesCreditNote()
...
function goJournal(row: ContabilidadSalesCreditNoteDTO) {
...
<template>
  <div class="px-3 sm:px-5 py-6 sm:py-8 space-y-6 max-w-[1600px] mx-auto">
    <PageHeader
      ...
      title="Notas de crédito de venta"
      subtitle="Reversan CxC e ingreso vinculado a comprobantes de cliente."
    >
      <template #actions>
      ...
      >
        <template #toolbar>
          <div class="flex-1 min-w-0">
            <SearchInput v-model="searchInput" placeholder="Buscar por NC o cliente…" />
          </div>
        </template>

        <template #row="{ row }">
          <td class="py-3 px-4 text-sm">{{ (row as ContabilidadSalesCreditNoteDTO).issueDate }}</td>
          <td class="py-3 px-4 font-mono text-sm">{{ (row as ContabilidadSalesCreditNoteDTO).fullNumber }}</td>
          <td class="py-3 px-4 text-sm">{{ (row as ContabilidadSalesCreditNoteDTO).customerName }}</td>
          <td class="py-3 px-4 text-sm font-mono">
            {{ (row as ContabilidadSalesCreditNoteDTO).invoiceFullNumber ?? '—' }}
          </td>
          <td class="py-3 px-4 text-sm text-right font-mono">
            {{ formatPen((row as ContabilidadSalesCreditNoteDTO).totalAmount) }}
          </td>
          <td class="py-3 px-4 text-right">
            <BaseButton
              v-if="(row as ContabilidadSalesCreditNoteDTO).journalEntryId"
              variant="secondary"
              size="sm"
              @click="goJournal(row as ContabilidadSalesCreditNoteDTO)"
            >
              Asiento
            </BaseButton>
          </td>
