...
import type { ContabilidadPurchaseCreditNoteDTO } from '../../domain/purchases.types'
...
const { data, isLoading, refetch } = useContabilidadPurchaseCreditNotes(listParams)
const rows = computed(() => data.value?.creditNotes ?? [])
...
const { mutate: createCreditNote, isPending: saving } = useContabilidadCreatePurchaseCreditNote()
...
function goJournal(row: ContabilidadPurchaseCreditNoteDTO) {
...
<template>
  <div class="px-3 sm:px-5 py-6 sm:py-8 space-y-6 max-w-[1600px] mx-auto">
    <PageHeader
      ...
      subtitle="Reversan CxP y crédito fiscal vinculado a facturas de proveedor."
    >
      <template #actions>
    ...
    >
      <DataTable
        ...
        :data="rows"
        :loading="isLoading"
        empty-text="Sin notas de crédito en este periodo."
        row-key="id"
      >
        <template #toolbar>
          <div class="flex-1 min-w-0">
            <SearchInput v-model="searchInput" placeholder="Buscar por NC o proveedor…" />
          </div>
        </template>

        <template #row="{ row }">
          <td class="py-3 px-4 text-sm">{{ (row as ContabilidadPurchaseCreditNoteDTO).issueDate }}</td>
          <td class="py-3 px-4 font-mono text-sm">{{ (row as ContabilidadPurchaseCreditNoteDTO).fullNumber }}</td>
          <td class="py-3 px-4 text-sm">{{ (row as ContabilidadPurchaseCreditNoteDTO).supplierName }}</td>
          <td class="py-3 px-4 text-sm font-mono">
            {{ (row as ContabilidadPurchaseCreditNoteDTO).invoiceFullNumber ?? '—' }}
          </td>
          <td class="py-3 px-4 text-sm text-right font-mono">
            {{ formatPen((row as ContabilidadPurchaseCreditNoteDTO).totalAmount) }}
          </td>
          <td class="py-3 px-4 text-right">
            <BaseButton
              v-if="(row as ContabilidadPurchaseCreditNoteDTO).journalEntryId"
              variant="secondary"
              size="sm"
              @click="goJournal(row as ContabilidadPurchaseCreditNoteDTO)"
            >
              Asiento
            </BaseButton>
          </td>
    ...
    <BaseModal v-model="modalOpen" title="Registrar nota de crédito" size="md">
